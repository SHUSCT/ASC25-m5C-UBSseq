...
DATA={}
step_start=$(date +%s.%N)
cutseq ./${DATA}/${DATA}.fastq -t 20 -A INLINE -m 20 --trim-polyA --ensure-inline-barcode \
    -o ./${DATA}/${DATA}.fastq_cut \
    -s ./${DATA}/${DATA}.fastq_tooshort \
    -u ./${DATA}/${DATA}.fastq_untrimmed 2> /dev/null
step_end=$(date +%s.%N)
...
hisat-3n/hisat-3n --index ./ref/index/hisat3n/Homo_sapiens.GRCh38.ncrna \
                  ...
                  -p 18  --base-change C,T --mp 8,2 --no-spliced-alignment --directional-mapping \
                  | samtools/samtools view -@ 2 -e '!flag.unmap' -O BAM -U ./${DATA}/${DATA}.ncrna.unmapped.bam \
                                                                           -o ./${DATA}/${DATA}.ncrna.mapped.bam
step_end=$(date +%s.%N)
echo "hisat-3n rRNA, tRNA filtering took $(echo "$step_end - $step_start" | bc) seconds."

step_start=$(date +%s.%N)
samtools/samtools fastq -@ 20 -O ./${DATA}/${DATA}.ncrna.unmapped.bam > ./${DATA}/${DATA}.mRNA.fastq
step_end=$(date +%s.%N)
echo "samtools fastq took $(echo "$step_end - $step_start" | bc) seconds."

step_start=$(date +%s.%N)
hisat-3n/hisat-3n --index ./ref/index/hisat3n/Homo_sapiens.GRCh38.dna.primary_assembly -p 18 \
                  ...
                  | samtools/samtools view -@ 2 -e '!flag.unmap' -O BAM -U ./${DATA}/${DATA}.mRNA.genome.unmapped.bam \
                                                                         -o ./${DATA}/${DATA}.mRNA.genome.mapped.bam
step_end=$(date +%s.%N)
echo "hisat-3n genome alignment took $(echo "$step_end - $step_start" | bc) seconds."

step_start=$(date +%s.%N)
samtools/samtools sort -@ 20 --write-index  -O BAM -o ./${DATA}/${DATA}.mRNA.genome.mapped.sorted.bam ./${DATA}/${DATA}.mRNA.genome.mapped.bam
step_end=$(date +%s.%N)
echo "samtools sort took $(echo "$step_end - $step_start" | bc) seconds."

step_start=$(date +%s.%N)
samtools/samtools view -@ 20 -F 3980 -c ./${DATA}/${DATA}.mRNA.genome.mapped.sorted.bam >./${DATA}/${DATA}.mRNA.genome.mapped.sorted.bam.tsv
step_end=$(date +%s.%N)
echo "samtools view took $(echo "$step_end - $step_start" | bc) seconds."

step_start=$(date +%s.%N)
java -server -Xms8G -Xmx40G -Xss100M -Djava.io.tmpdir=./${DATA} \
     -jar ./UMICollapse/umicollapse.jar bam -t 16 --data naive --merge avgqual --two-pass \
...
step_start=$(date +%s.%N)
samtools/samtools view -@ 20 -e "[XM] * 20 <= (qlen-sclen) && [Zf] <= 3 && 3 * [Zf] <= [Zf] + [Yf]" -O BAM -o ./${DATA}/${DATA}.mRNA.genome.mapped.sorted.dedup.filtered.bam ./${DATA}/${DATA}.mRNA.genome.mapped.sorted.dedup.bam
step_end=$(date +%s.%N)
echo "samtools filter took $(echo "$step_end - $step_start" | bc) seconds."

step_start=$(date +%s.%N)
samtools/samtools index -@ 20 ./${DATA}/${DATA}.mRNA.genome.mapped.sorted.dedup.bam ./${DATA}/${DATA}.mRNA.genome.mapped.sorted.dedup.bam.bai
step_end=$(date +%s.%N)
